...
class LocalizedFiltering(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.lf_conv2d_group = args.lf_conv2d_group
        self.lf_conv2d_num_pad = args.lf_conv2d_num_pad
...
class ParallelAttention(MegatronModule):
    ...
    def _allocate_memory_hidden_states(self, batch_size):
        return torch.empty(
            2,
            batch_size,
            self.hidden_size,
    ...
    def forward(self, hidden_states, attention_mask, enc_position_ids,
        ...
        if self.attention_type == AttnType.self_attn:
            if self.use_lf_gate:
                ...
                value_layer, _ = self.get_value(hidden_states)
                # [sq, b, hp] --> [sq, b, np, hn]
                ...
                mixed_qk_layer, _ = self.get_query_key(hidden_states)
                ...
                (query_layer,
                 key_layer) = tensor_parallel.split_tensor_along_last_dim(mixed_qk_layer, 2)            
            else:
        ...
        if rotary_pos_emb is not None:
            q_pos_emb, k_pos_emb = rotary_pos_emb
            query_layer = apply_rotary_pos_emb(query_layer, q_pos_emb, enc_position_ids)
            key_layer = apply_rotary_pos_emb(key_layer, k_pos_emb, enc_position_ids)
...
class ParallelTransformer(MegatronModule):
    ...
    def __init__(self, config,
        ...
        # Number of layers.
        if args.pipeline_model_parallel_method == 'uniform':
             self.num_layers = _get_num_layers(args, model_type,
                                              layer_type==LayerType.decoder)
        else:
             self.num_layers = args.pipeline_blocks[mpu.get_pipeline_model_parallel_rank()]
        ...
        if config.virtual_pipeline_model_parallel_size is not None:
            ...
            if args.model_type == ModelType.encoder_and_decoder and \
                ...
                assert args.pipeline_model_parallel_method != 'block', 'encoder_and_decoder model type not support user define pipeline_model_parallel_methood'
                ...
                if mpu.get_pipeline_model_parallel_rank() == 0:
                    ...
                    offset = sum(args.pipeline_blocks[:mpu.get_pipeline_model_parallel_rank()])
        ...
        if self.post_process and self.post_layer_norm:
            ...
            if args.norm_dtype == "LayerNorm":
                ...
                self.final_layernorm = RMSNorm(config.hidden_size, eps=1e-6)

    def _get_layer(self, layer_number):
        return self.layers[layer_number]

    def _checkpointed_forward(self, hidden_states, attention_mask, enc_position_ids,
            ...
            while l < self.num_layers:
                if self.transformer_impl == 'transformer_engine':
                    hidden_states = transformer_engine.pytorch.distributed.checkpoint(
                        ...
                        hidden_states, attention_mask, enc_position_ids, encoder_output,
