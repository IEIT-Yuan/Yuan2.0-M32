...
def build_tokenizer(args):
    ...
    if args.tokenizer_type == 'BertWordPieceLowerCase':
        ...
        tokenizer.add_tokens(['<sep>', '<pad>','<mask>','<predict>','<FIM_SUFFIX>','<FIM_PREFIX>','<FIM_MIDDLE>','<commit_before>','<commit_msg>','<commit_after>','<jupyter_start>','<jupyter_text>','<jupyter_code>','<jupyter_output>','<empty_output>', '<repo_name>', '<file_sep>'], special_tokens = True)
    else:
...
class _EncDecTokenizer(AbstractTokenizer):
    """Original GPT2 BPE tokenizer."""
    def __init__(self, vocab_file ):
        name = 'GPT2 enc dec'
        super().__init__(name)
        from .tokenization_enc_dec import EncDecTokenizer
        self.tokenizer = EncDecTokenizer(vocab_file)
